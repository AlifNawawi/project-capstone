...
export const registerModel = {
  async register(username, email, password, gender) {
    ...
    try {
      ...
      return {
        success: false,
        error: data.details || data.error || "Registrasi gagal",
      };
    } catch (error) {
  ...
  async verifyToken(token) {
    try {
      ...
      console.error(
        "Error dari server di verifyToken:",
        data.error,
        data.details
      );
      return {
        success: false,
        error: data.details || data.error || "Verifikasi token gagal",
      };
    } catch (error) {
  ...
  async updateUserData(uid, username, gender) {
    try {
      console.log(
        "Mengirim data ke update-user:",
        JSON.stringify({ uid, username, gender }, null, 2)
      );
      ...
      console.error(
        "Error dari server di updateUserData:",
        data.error,
        data.details
      );
      return {
        success: false,
        error: data.details || data.error || "Gagal memperbarui data pengguna",
      };
    } catch (error) {
